    ...
    <section className="overflow-hidden border border-[var(--line-strong)] bg-[#fffdfb]">
      ...
      <div className="p-6">
        <div className="mb-6 flex items-center gap-3 border-b border-slate-200 pb-4">
          ...
          <div>
            <p className="eyebrow text-forest-700">Regional Engagement</p>
        ...
        {visibleCounties.length > 0 ? (
          <div className="space-y-4">
            {visibleCounties.map((county) => {
              ...
              return (
                <div key={`${county.billId ?? 'global'}-${county.county}`} className="rounded-xl border border-slate-300 bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span className="text-sm font-semibold text-slate-950">{county.county}</span>
                      <span className={`ml-2 rounded-xl px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] ${sentimentClass}`}>
                        {county.sentiment}
                      </span>
                    </div>
                    <span className="metric-mono text-xs font-semibold text-slate-500">
