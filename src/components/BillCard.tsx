...
export default function BillCard({ bill, petition }: Props) {
  ...
  return (
    <article className="surface-card group flex h-full flex-col overflow-hidden transition duration-300 hover:-translate-y-0.5 hover:shadow-(--shadow-lift)">
      ...
      <div className="flex-1 p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex flex-wrap gap-2">
            ...
            {bill.isHot && (
              <span className="rounded-xl bg-warning-soft px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-warning">
                Hot
              </span>
            )}
            {pdfUrl && (
              <span className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-600">
                <FileText size={12} />
                PDF available
              </span>
            )}
          </div>
          <span className="metric-mono text-xs uppercase tracking-[0.22em] text-slate-500">{introducedAt}</span>
        </div>
        ...
        {livePetition && (
          <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3 text-xs font-semibold">
              ...
              <span className="metric-mono text-brand-strong">
      ...
      <div className="grid grid-cols-2 gap-3 border-t border-slate-100 bg-slate-50/80 p-4">
        <Link
          href={`/bills/${bill.id}`}
          className="inline-flex items-center justify-center rounded-xl bg-brand px-4 py-3 text-sm font-semibold text-white transition hover:bg-brand-strong"
        >
          Open bill
        </Link>
        ...
        {bill.parliamentUrl ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-brand/20 hover:text-brand-strong"
          >
          ...
          <Link
            ...
            className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-brand/20 hover:text-brand-strong"
          >
            Participate
          </Link>
        )}
